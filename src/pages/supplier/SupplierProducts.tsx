...
export default function SupplierProducts({ onLogout }: { onLogout: () => void }) {
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      setNotice(saved.isActive ? (profile?.verificationStatus === 'APPROVED' ? 'Đã đăng bán trên Tìm nguồn sỉ.' : 'Đã lưu sản phẩm. Gian hàng cần được xác minh trước khi mở bán.') : 'Đã lưu sản phẩm ở trạng thái ẩn.');
    } catch (issue) {
      setError(getApiError(issue)); setFields(getApiFieldErrors(issue, ['name', 'packaging', 'wholesalePrice', 'stockQty', 'moq', 'isActive', 'category', 'imageUrl']));
  ...
  async function toggle(item: SupplierProduct) {
    ...
    try {
      const saved = await updateSupplierProduct(item.id, { category: item.category, imageUrl: item.imageUrl, name: item.name, packaging: item.packaging, wholesalePrice: item.wholesalePrice, stockQty: item.stockQty, moq: item.moq, isActive: !item.isActive }, item.updatedAt);
      remember(saved);
      setNotice(saved.isActive ? (profile?.verificationStatus === 'APPROVED' ? 'Đã đăng lại sản phẩm cho chủ tạp hóa xem.' : 'Đã lưu. Chờ gian hàng được xác minh để mở bán.') : 'Đã ẩn sản phẩm khỏi Tìm nguồn sỉ.');
